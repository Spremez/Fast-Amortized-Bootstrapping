...
out_dir="${STAGE44_OUT_DIR:-repro/stage44_external_unlock_reprobe}"
run_native_bench="${STAGE44_RUN_NATIVE_BENCH:-1}"
...
stage44_log="$out_dir/stage44_log_builder.log"
...
native_status="$(
  awk -F, '$1 == "hardware_counter_gate" {print $2}' "$out_dir/native_perf_gate/summary.csv"
)"
...
"$python_bin" scripts/build_stage44_external_unlock_reprobe.py \
  --out-dir "$out_dir" > "$stage44_log" 2>&1

printf 'Stage 44 external unlock re-probe summary: %s\n' "$out_dir/summary.csv"
