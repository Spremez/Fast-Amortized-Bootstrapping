...
variants="${STAGE22_VARIANTS:-generic specialized}"
kernel_runs="${STAGE22_KERNEL_RUNS:-1}"
full_runs="${STAGE22_FULL_SAB_RUNS:-1}"
full_r_values="${STAGE22_FULL_SAB_R_VALUES:-2 4}"
full_reps="${SAB_PVW_BENCH_REPS:-1}"
fft_lib="${FFT_LIB:-spqlios_avx512}"
kernel_key="${STAGE22_KERNEL_KEY:-BINARY}"
kernel_param="${STAGE22_KERNEL_PARAM:-SET_2_3}"
full_key="${KEY:-BINARY}"
full_param="${PARAM:-SET_2_3_2048}"
jobs="${JOBS:-$(nproc)}"
out_dir="${STAGE22_OUT_DIR:-repro/stage22_mat_avx512_limit_audit_runs${kernel_runs}_full${full_runs}}"
...
printf 'variant,run,label,r,reps,phase_sum_avg_us,decompose_avg_us,decompose_pct,dft_avg_us,dft_pct,mul_avg_us,mul_pct,source_log\n' > "$phase_csv"
...
mat_flag_for_variant() {
  case "$1" in
    generic) printf 'false' ;;
    specialized) printf 'true' ;;
    *)
      printf 'unknown Stage22 variant: %s\n' "$1" >&2
      exit 1
      ;;
  esac
}
...
for variant in $variants; do
  mat_flag="$(mat_flag_for_variant "$variant")"
  variant_out="$out_dir/$variant"
  mkdir -p "$variant_out"

  make clean
  make FFT_LIB="$fft_lib" MAT_TRGSW_AVX512_SMALLR_SPECIALIZED="$mat_flag" \
  ...
  for run_idx in $(seq 0 "$((kernel_runs - 1))"); do
    ...
    while IFS= read -r line; do
      ...
      printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
        ...
        "$(extract_field "$line" mul_pct)" \
        "$log_file" >> "$phase_csv"
    done < <(grep '^EP_BREAKDOWN ' "$log_file")
  done

  if [[ "$full_runs" -gt 0 ]]; then
    for r in $full_r_values; do
      full_variant_out="$variant_out/full_r${r}"
      mkdir -p "$full_variant_out"
      make clean
      make FFT_LIB="$fft_lib" MAT_TRGSW_AVX512_SMALLR_SPECIALIZED="$mat_flag" \
        SAB_PVW_ACTIVE_BUFFER_FUSION=true \
        SAB_PVW_BENCH=true SAB_PVW_BENCH_R="$r" \
...
printf 'Stage 22 MAT AVX512 audit: %s\n' "$out_dir"
