...
OUT="${STAGE341_OUT_DIR:-$ROOT/repro/stage341_parameter_matrix_smoke}"
RAW="$OUT/raw"
JOBS="${JOBS:-$(nproc)}"
PARAM_VALUE="${PARAM:-SET_2_3_2048}"
R_VALUE="${STAGE341_R:-2}"
FFT_LIB_VALUE="${FFT_LIB:-spqlios_avx512}"
PERF_RUNS="${STAGE341_PERF_RUNS:-1}"
PERF_REPS="${STAGE341_PERF_REPS:-1}"
NOISE_TRIALS="${STAGE341_NOISE_TRIALS:-1}"
...
STAGE341_OUT_DIR="$OUT" python3 scripts/build_stage341_parameter_matrix_smoke.py
