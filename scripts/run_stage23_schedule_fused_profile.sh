...
r_values="${STAGE23_SCHEDULE_PROFILE_R_VALUES:-2 4}"
runs="${STAGE23_SCHEDULE_PROFILE_RUNS:-1}"
...
schedule_fused="${SAB_PVW_SCHEDULE_FUSED_CMUX:-true}"
key="${KEY:-BINARY}"
param="${PARAM:-SET_2_3_2048}"
jobs="${JOBS:-$(nproc)}"
out_dir="${STAGE23_SCHEDULE_PROFILE_OUT_DIR:-repro/stage23_schedule_fused_profile_${fft_lib}_runs${runs}}"
...
printf 'r,run,status,pvw_avg_us,scalar_repeated_avg_us,speedup,in_N,h,r_prec,rgsw_monomial_calls,cmux_calls,expected_cmux,ncmux_calls,expected_ncmux,mat_ep_calls,sub_a_calls,expected_sub_a,copyback_calls,expected_copyback,schedule_fused_cmux_calls,expected_schedule_fused_cmux,schedule_fused_ncmux_calls,expected_schedule_fused_ncmux,cmux_from_dft_calls,cmux_add_calls,source_log\n' > "$summary_csv"
...
for r in $r_values; do
  ...
  make FFT_LIB="$fft_lib" \
    ...
    SAB_PVW_SCHEDULE_FUSED_CMUX="$schedule_fused" \
  ...
  for run_idx in $(seq 0 "$((runs - 1))"); do
    ...
    schedule_fused_cmux_calls="$(extract_field "$profile_line" schedule_fused_cmux_calls)"
    schedule_fused_ncmux_calls="$(extract_field "$profile_line" schedule_fused_ncmux_calls)"
    ...
    if [[ -z "$in_N" || -z "$h" || -z "$r_prec" ||
          ...
          -z "$copyback_calls" ||
          -z "$schedule_fused_cmux_calls" ||
          -z "$schedule_fused_ncmux_calls" ||
          -z "$cmux_from_dft_calls" || -z "$cmux_add_calls" ]]; then
      printf 'failed to parse Stage 23 profile line in %s: %s\n' \
    ...
    expected_copyback=0
    expected_schedule_fused_cmux="$((expected_cmux - expected_ncmux))"
    expected_schedule_fused_ncmux="$expected_ncmux"
    ...
    if [[ "$rgsw_calls" -ne "$expected_rgsw" ||
          ...
          "$copyback_calls" -ne "$expected_copyback" ||
          "$schedule_fused_cmux_calls" -ne "$expected_schedule_fused_cmux" ||
          "$schedule_fused_ncmux_calls" -ne "$expected_schedule_fused_ncmux" ]]; then
      status="COUNT_FAIL"
      failures="$((failures + 1))"
    fi
    ...
    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
      ...
      "$copyback_calls" "$expected_copyback" \
      "$schedule_fused_cmux_calls" "$expected_schedule_fused_cmux" \
      "$schedule_fused_ncmux_calls" "$expected_schedule_fused_ncmux" \
      "$cmux_from_dft_calls" "$cmux_add_calls" "$log_file" >> "$summary_csv"
  done
done

printf 'Stage 23 schedule-fused profile summary: %s\n' "$summary_csv"

if [[ "$failures" -ne 0 ]]; then
  printf 'Stage 23 schedule-fused profile failed with %s count mismatch(es)\n' \
    "$failures" >&2
  exit 1
fi
