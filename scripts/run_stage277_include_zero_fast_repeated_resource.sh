...
RAW="$ROOT/repro/stage277_include_zero_fast_repeated_resource/raw"
JOBS="${JOBS:-$(nproc)}"
...
common_bench_flags=(
  ...
  SAB_PVW_NONBINARY_BENCH_REPS=2
...
run_bench_case() {
  ...
  shift
  local extra_flags=("$@")
  local stem="${variant}_include_zero_r4_reps2"
...
run_resource_case() {
  local stem="include_zero_coeff_one_fast_resource_trials1"

  make clean >"$RAW/${stem}_clean.log" 2>&1
  make FFT_LIB=ffnt KEY=BINARY PARAM=SET_2_3 \
    SAB_PVW_INCLUDE_ZERO_FAST_RESOURCE_TEST=true \
    SAB_PVW_INCLUDE_ZERO_FAST_RESOURCE_TRIALS=1 \
