...
def build_rows() -> List[Dict[str, str]]:
    ...
    if stage73_closed:
        optional_notes.append("Stage73 final-recheck Stage72 integration")
    if stage74_closed:
    ...
    if stage77_closed:
        optional_notes.append("Stage77 r>4 fused MAT smoke")
    if stage78_closed:
    ...
    if stage81_closed:
        optional_notes.append("Stage81 next-variant triage")
    if stage82_closed:
        optional_notes.append("Stage82 post-H11 profile")
    if stage83_closed:
        optional_notes.append("Stage83 MAT body design check")
    if stage84_closed:
        optional_notes.append("Stage84 H13 r=6 tile-sweep preflight")
    if stage86_closed:
    ...
    if stage89_closed:
        optional_notes.append("Stage89 H14 promotion policy integration")
    if stage90_closed:
        optional_notes.append("Stage90 external claim unlock")
    if stage91_closed:
    ...
    if stage96_closed:
        optional_notes.append("Stage96 upstream delta audit")
    if stage97_closed:
