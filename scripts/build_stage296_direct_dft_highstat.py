#!/usr/bin/env python3
"""Stage296: high-stat direct-DFT full-SAB refresh."""
...
OUT = ROOT / "repro" / "stage296_direct_dft_highstat"
RAW = OUT / "raw"
DOC = ROOT / "docs" / "stage296_direct_dft_highstat.md"
THEORY = ROOT / "theory_checks" / "stage296_direct_dft_highstat_model.md"
VARIANT = ROOT / "algorithm_variants" / "mat_rlwe_sab_stage296_direct_dft_highstat.md"
PLAN = ROOT / "experiments" / "stage296_direct_dft_highstat_plan.md"
RUNNER = ROOT / "scripts" / "run_stage296_direct_dft_highstat.sh"
BUILDER = ROOT / "scripts" / "build_stage296_direct_dft_highstat.py"
...
REPORT = OUT / "stage296_report.md"
ARTIFACT = OUT / "artifact_index.csv"

DECISION_PASS = "PASS_STAGE296_DIRECT_DFT_HIGHSTAT_LOCAL"
DECISION_NEUTRAL = "NEUTRAL_STAGE296_DIRECT_DFT_HIGHSTAT_CI_OR_EFFECT_WEAK"
DECISION_FAIL = "FAIL_STAGE296_DIRECT_DFT_HIGHSTAT"
...
def main() -> None:
    ...
    stage295_ok = "PASS_STAGE295_DIRECT_DFT_STATS_REFRESH_HIGHSTAT_PENDING" in read_text(ROOT / "repro" / "stage295_direct_dft_stats_refresh" / "proof_gate.csv")
    perf_ok = (
    ...
    if stage295_ok and perf_ok and noise_ok:
        decision = DECISION_PASS
    elif stage295_ok and control.get("correctness") == "Pass" and direct.get("correctness") == "Pass" and noise.get("status") == "pass":
        decision = DECISION_NEUTRAL
    else:
        decision = DECISION_FAIL

    proof_rows = [
        {"gate": "G1_prior_stage295", "status": "PASS" if stage295_ok else "FAIL", "metric": "Stage295 decision", "value": "positive" if stage295_ok else "missing", "interpretation": "Stage296 can only extend a positive Stage295 refresh."},
        ...
        {"gate": "G7_decision", "status": decision, "metric": "stage decision", "value": decision, "interpretation": "Controls Stage297 route."},
    ]
    ...
    write_csv(NEXT, ["priority", "route", "entry_condition", "gate", "failure_action"], [
        {"priority": "P0", "route": "stage297_native_or_resource_matrix", "entry_condition": decision, "gate": "native counter/resource-backed attribution or 10+ run repeated on native Linux.", "failure_action": "Keep Stage296 local only."},
        {"priority": "P1", "route": "stage297_stagewise_noise", "entry_condition": "paper claim needs noise internalization", "gate": "target-size per-stage PVW/scalar phase/noise equivalence.", "failure_action": "Only claim final-output equivalence."},
    ])

    command_text = """# Stage296 Reproduction Commands
...
"""
    ...
    report = f"""# Stage296 Direct DFT High-Stat Campaign
...
"""
    ...
    write_text(THEORY, """# Stage296 Direct DFT High-Stat Model
...
""")
    write_text(VARIANT, """# Stage296 Direct DFT High-Stat Variant
...
This remains the Stage292 direct-DFT candidate. Stage296 only strengthens the
evidence base and does not change scalar SAB or selected-control PVW-SAB.
""")
    write_text(PLAN, """# Stage296 Experiment Plan
...
""")
    ...
    append_once(GOAL, "<!-- stage296-direct-dft-highstat -->", f"""<!-- stage296-direct-dft-highstat -->
### Stage296 direct DFT high-stat campaign

`{decision}` expands the direct-DFT candidate to a local 10-run/10-trial
...
""")
    append_once(ROADMAP, "## Stage 296: Direct DFT High-Stat Campaign", f"""## Stage 296: Direct DFT High-Stat Campaign
...
Status: `{decision}`.
""")
    append_once(HYPOTHESES, "H296_direct_dft_highstat:", f"""H296_direct_dft_highstat:
  status: {decision}
  ...
  evidence:
    ...
    - repro/stage296_direct_dft_highstat/proof_gate.csv
    - docs/stage296_direct_dft_highstat.md
  conclusion: >
    Stage296 tests whether direct-DFT complete-SAB throughput persists under a
    10-run/10-trial local campaign. It is still below paper-grade until native
    counter/resource, parameter, and stage-wise noise gates are complete.
""")
    append_once(MANIFEST, "- stage296_direct_dft_highstat:", """- stage296_direct_dft_highstat:
  - `docs/stage296_direct_dft_highstat.md`
...
""")
    append_once(CHECKLIST, "<!-- stage296-direct-dft-highstat-checklist -->", f"""<!-- stage296-direct-dft-highstat-checklist -->
- [x] Stage296 records `{decision}` for direct DFT high-stat local complete-SAB evidence.
""")
    append_run_log({
        "run_id": "stage296-direct-dft-highstat-001",
        "date": "2026-07-04",
        "git_ref": git_head(),
        "stage": "Stage 296",
        "backend": "spqlios_avx512-local",
        "command": "STAGE296_PERF_RUNS=10 STAGE296_NOISE_TRIALS=10 FFT_LIB=spqlios_avx512 PARAM=SET_2_3_2048 bash scripts/run_stage296_direct_dft_highstat.sh",
        ...
        "artifacts": "docs/stage296_direct_dft_highstat.md; repro/stage296_direct_dft_highstat/proof_gate.csv",
    })
