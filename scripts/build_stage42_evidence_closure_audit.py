...
Stage86 control-plane/design extensions are internally consistent. It does not run
...
def check_run_log() -> List[Dict[str, str]]:
    ...
    detail = (
        f"stages 19-62 registered; stage41 status={stage41_status}; stage66 status={stage66_status}; stage67 status={stage67_status}; stage68 status={stage68_status}; stage69 status={stage69_status}; stage70 status={stage70_status}; stage71 status={stage71_status}; stage72 status={stage72_status}; stage73 status={stage73_status}; stage74 status={stage74_status}; stage75 status={stage75_status}; stage76 status={stage76_status}; stage77 status={stage77_status}; stage78 status={stage78_status}; stage79 status={stage79_status}; stage80 status={stage80_status}; stage81 status={stage81_status}; stage82 status={stage82_status}; stage83 status={stage83_status}; stage84 status={stage84_status}; stage86 status={stage86_status}"
        if ok
...
def check_required_files() -> List[Dict[str, str]]:
    ...
    return [
        row(
            ...
            "; ".join(REQUIRED_FILES),
            "all required Stage 41-62 plus Stage64A/Stage65A/Stage66A/Stage67/Stage68/Stage69/Stage70/Stage71/Stage72/Stage73/Stage74/Stage75/Stage76/Stage77/Stage78/Stage79/Stage80/Stage81/Stage82/Stage83/Stage84/Stage86 files exist" if not missing else f"missing={missing}",
            "Restore missing Stage 41-62, Stage64A, Stage65A, Stage66A, Stage67, Stage68, Stage69, Stage70, Stage71, Stage72, Stage73, Stage74, Stage75, Stage76, Stage77, Stage78, Stage79, Stage80, Stage81, Stage82, Stage83, Stage84, or Stage86 control-plane artifacts.",
        )
    ]
...
def write_md(rows: List[Dict[str, str]]) -> None:
    ...
    lines = [
        ...
        "final-recheck Stage66A integration, and Stage68-86 control-plane",
