...
STAGE105_GOAL_COMPLETION_AUDIT = (
    ROOT / "repro" / "stage105_goal_completion_audit" / "summary.csv"
)
...
def check_stage104_post_external_final_package() -> List[Dict[str, str]]:
    rows = {r.get("gate"): r for r in read_csv(STAGE104_POST_EXTERNAL_FINAL_PACKAGE)}
    expected = {
    ...
    for gate, expected_status in expected.items():
        ...
        if actual != expected_status:
            problems.append(f"{gate}:status={actual}")
    detail = (
        "Stage104 refreshes the post-external final scoped package and preserves claim boundaries"
        if not problems and rows
        else "; ".join(problems) or "Stage104 summary missing"
    )
    return [
        row(
            ...
            "claim_scope",
            pass_fail(not problems and bool(rows)),
            STAGE104_POST_EXTERNAL_FINAL_PACKAGE.relative_to(ROOT).as_posix(),
            detail,
            "Rerun Stage104 before treating the post-external package as current.",
        )
    ]


def check_stage105_goal_completion_audit() -> List[Dict[str, str]]:
    rows = {r.get("gate"): r for r in read_csv(STAGE105_GOAL_COMPLETION_AUDIT)}
    expected = {
        "stage105_requirement_coverage": "PASS",
        "stage105_claim_limit_guard": "PASS",
        "stage105_decision": "PASS_STAGE105_SCOPED_GOAL_COMPLETE_STRONGER_CLAIMS_BLOCKED",
    }
    problems = []
    for gate, expected_status in expected.items():
        actual = rows.get(gate, {}).get("status", "MISSING")
        if actual != expected_status:
            problems.append(f"{gate}:status={actual}")
    detail = (
        "Stage105 proves scoped goal completion and preserves stronger claim boundaries"
        if not problems and rows
        else "; ".join(problems) or "Stage105 summary missing"
    )
    return [
        row(
            "S42-STAGE105-GOAL-COMPLETION-AUDIT",
            "claim_scope",
            pass_fail(not problems and bool(rows)),
            STAGE105_GOAL_COMPLETION_AUDIT.relative_to(ROOT).as_posix(),
            detail,
            "Rerun Stage105 before treating the active scoped goal as complete.",
        )
    ]
...
def check_run_log() -> List[Dict[str, str]]:
    ...
    stage104_status = "MISSING"
    for r in rows:
        if r.get("run_id") == "stage104-post-external-final-package-001":
            stage104_status = r.get("status", "MISSING")
    stage105_status = "MISSING"
    for r in rows:
        if r.get("run_id") == "stage105-goal-completion-audit-001":
            stage105_status = r.get("status", "MISSING")
    ok = (
        ok
...
def build_rows() -> List[Dict[str, str]]:
    ...
    for fn in [
        ...
        check_stage104_post_external_final_package,
        check_stage105_goal_completion_audit,
    ...
    checks.append(
        row(
            ...
            "Fix all failed checks before relying on the Stage 19-62 plus  evidence closure.",
        )
    )
    return checks
