...
1. 噪声侧: 密钥噪声项系数从 σ·2^{27.1}（stock）降到 σ·2^{q+3.5}（每外积，rms，N=2048），等效 σ 余量 +2(23−q) bit —— 用于吸收 2026/279 的稀疏密钥安全差（保守 15 bit），σ 上调 + aut-KS gadget 联动细化（SQKS_AUT_*）下已实测: σ_out+15 bit 时 SQ(q=16) gate Pass 而 stock scalar 失效。
2. 核侧（v3）: 去掉 gadget 分解、重缩放与 CMUX 重组融合为单遍、零拷贝 ping-pong；实测 9 轮同二进制比值中位 −4.5%、min-of-9 −7.3%（快于 scalar），FFD 计数不变（4 FFT + 4 pw / 外积）。服务器 AVX-512 高统计确认留 stage357。
