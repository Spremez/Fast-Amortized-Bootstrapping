...
\begin{keybox}
\textbf{完成度（截至本资料生成）}：安全组合定案、终版性能主表、r-扩展性（r=2/4/6）、层安全审计、容量曲线（h=41/$\sigma$+11 口径）、TFHE-rs 同机锚点、occupancy/break-even、DFR 三层证据、多参数集修正推导均已\textbf{完成}。在途（不改变主表方向）：r=8 静机复测、postops 在最终参数下的交替翻转分诊、10-trial 噪声@最终参数、E-PF 部分融合 A/B、4096/5-bit 实测收割（首项已 Pass：4096 修正点 h=42 门测试 0/4096 失配）。
\end{keybox}
...
$h{=}42$ 是 $n{=}2048$ 输入环族（跨精度）的定案，\textbf{非全局常数}：逐环以三联约束 $T3'\ge128\ \wedge\ \mathrm{MITM}\ge128\ \wedge\ \mathrm{BSK}\ge128$ 求解。$n{=}4096$：原 $h{=}32$ 坍至 $117.5$，修正为 $h{=}42@\mathrm{t}8$（$T3'{=}154.1$，实测门测试已通过）；$n{=}8192$：修正为 $h{=}34@\mathrm{t}10$（$146.6$，实测排队）。
...
\begin{table}[htbp]
\centering
\caption{r-lane（$r{=}4$，Final）：同一盲旋转服务 $2048\times4$ 查询}
\label{tab:rlane}
\small
...
方案 & ms/msg/lut & 内部比值 \\
\midrule
stock r-lane（686 矩阵化） & 5.25 & $1.00\times$ \\
SQ r-lane & 5.29 & $1.01\times$ \\
\textbf{路径A（7 融合旗标）} & \textbf{4.32} & \textbf{$1.21\times$ vs stock；$1.16\times$ vs SQ} \\
\bottomrule
\end{tabular*}
\end{table}

同场次对 $4\times$686 标量（$52.4$ s）：路径A $1.48\times$，SQ 标量 $\times4$ 为 $1.33\times$。\textbf{安全修正的完整代价}（同机同场）：$h$ 由 39 增至 42 仅 $+6.5\%$（stock）/$+7.2\%$（SQ），$\sigma$ 不动。

\subsection{r-扩展性与正确性}

路径A 对 SQ 的逐进程背靠背比值：$r{=}2$: $1.117$（$2.10$ ms/msg/lut）；$r{=}4$: $1.16$；$r{=}6$: $1.10$。每 lane 成本随 $r$ 单调上升，摊销甜点 $r{=}2$--$4$；全部 $r$ 门测试通过（正确性与 $r$ 无关）。$r{=}8$ 静机复测在途。
...
同机（dell）TFHE-rs v1.8.0 默认 128-bit 经典参数：\textbf{9.560 ms/PBS}（30 次操作精确计数 330 次 PBS，解密校验通过；其参数未按 2026 攻击修正——已标注）。满占用吞吐对比 $4.32$ vs $9.56$ ms/查询 $= 2.2\times$；临界占用率 $\kappa^\ast = T_{\mathrm{batch}}/(nrT_{\mathrm{pbs}})$：$r{=}4$ 时 $18.4\%$，$r{=}1$ 时 $36.8\%$。
...
SQ 与 7 个融合旗标中作用于分解生命周期的部分\textbf{构建不兼容}（同一输出路径冲突，可复现的冲突矩阵）；$k$-sub 推广被蝶形拓扑排除（证明而非性能猜测）；数位宽度微变体已被 stage314 实测关闭（微核 $1.045\times$ 稀释至全 SAB $0.997\times$）；配对堆叠 EP 因成本模型修正而撤回。
