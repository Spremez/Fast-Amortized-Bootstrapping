...
\begin{keybox}
\textbf{完成度（截至本资料生成）}：安全组合（输入层定案）、终版性能主表、层安全审计、容量曲线（h=41/$\sigma$+11 口径）、TFHE-rs 同机锚点、occupancy/break-even、DFR 三层证据、多参数集修正（4096/5-bit 已\textbf{实测}）、stage368 全部五节（r=8 静机、E-PF A/B、S7 十次目标噪声审计门 \textbf{Pass}（10 trial $\times$ 81{,}920 点，pair 失败 0）、postops 翻转分诊结案、S5 实测）均已\textbf{完成}。\textbf{新开口（\S5.1 决策框）}：V5A2/A3 新估计器复算发现 BSK 层在交替符号诚实模型下 $2^{127.9}<128$，三个已实测修正选项待定夺。在途：8192 环族实测。\textbf{勘误}：早前 r-lane 表中路径A $4.32$ ms/msg 系误取 pathA 构建二进制的 stock 对照列（跨构建代码布局漂移实测达 $23\%$）；本版全部 r-lane 结论已改为\textbf{同构建内部对照}口径（表~\ref{tab:rlane}）。
\end{keybox}
...
{\sloppy 安全：\texttt{scripts/e1\_conditional\_entropy.py}、\texttt{scripts/e4\_v5*.sage}、dell \texttt{e4v5*.log}/\texttt{e3.log}。性能：\texttt{repro/dell\_phase2/}（主表）、\texttt{repro/stage358..367}（σ 梯度、r-扩展、噪声、容量、平衡档）。理论：\texttt{docs/paper\_ccf\_a/theory\_for\_review.md}、\texttt{review\_package.md}。实现：\texttt{src/sab\_sq.c}、\texttt{src/sab\_pvw\_sq.c}、\texttt{src/sab\_pvw.c}、\texttt{src/mosfhet/src/mattrgsw.c}。全部阶段有 git 提交与服务器日志双重溯源。\par}
