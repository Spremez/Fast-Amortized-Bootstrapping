...
| S59-R6-OPTIONAL-VARIANTS | optional local expansion | future algorithmic variants | READY_OPTIONAL_LOCAL_TRIAGE | Stage80 keeps H11 r=6 fused MAT as an explicit experimental flag only: complete-SAB/noise/resource pass, but the high-stat performance boundary does not justify promotion over the r=4 reference. Next local work is Stage81 variant triage; no default/path promotion or claim upgrade for r=6 fused MAT. |
