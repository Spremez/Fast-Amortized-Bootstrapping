...
|---|---|---|---|
| verification_input_commit | 3388259 | git rev-parse --short HEAD | Commit checked before writing verifier output artifacts. |
...
| stage42_run_log_rows | PASS | repro/run_log.csv | all Stage42/43/44/45/46 closure run rows are present |
| artifact_manifest_mentions | PASS | repro/artifact_manifest.md | Stage42 verifier, closure manifest, Stage44 re-probe, Stage45 refactor, and Stage46 WSL target smoke are registered |
