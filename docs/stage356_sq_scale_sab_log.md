...
3. **速度**: SQ 比 scalar 慢 1.0%（q=16: 26.78s vs 26.52s）～1.7%（q=23: 27.14s vs 26.69s），每自举、3 次重复、CI < 0.5%（WSL ffnt/haswell，MOSFHET_TEST_RNG_SEED=1 可复现）——落在 ±5% 门限内；机制卡"等速"预期成立，"更快"主张撤回（分解环与舍入环同量级，理论即如此）。AVX-512 服务器核上 decompose-elimination 与 SUB_DECOMP_FUSION 类融合的组合留 stage357。
