...
>
> **⚠️ 2026-09-11 更新（WS-8/WS-9）**：**+257 常数定律闭合**（stage419 §七）：
> C = 2^(log₂(2N)−prec−1) + 1 = modswitch(po) + 1，四参数点（out_N 2048/4096/1024 ×
> prec 2/3/5）× 双路径 × 256 槽全部精确；定理重述为 E(t) ≈ −modswitch(φ)+1（结构
> +1 的 S1 机制解释标注为待形式化）。**WS-9 定位+快假设否定**（stage421 §五d）：
> 每行成本 2-5 行持平、9 行 +34% 悬崖；"加 R4/RGT4 旗标"假设被同会话 A/B 否定
> （现有瓦片内核为 r-lane 布局调优，与本管线不匹配）——bodies=8 需新内核开发，
> **不阻塞 WS-10**（论文锚点 = bodies=4 三轴 1.60×±1%）。下一动作：WS-10 论文并入
> （素材全齐：stage419 定理+证据链 / stage420 三表 / stage421 §五b-d）。

> **⚠️ 2026-09-10 深夜终版（stage419-421 全闭合）**：在下方导航基础上新增——
> **WS-6 密码学真实预对齐闭合**（真 keyswitch 版：等价/功能门全过、余量同阶、
> KS 成本 0.002%）；**WS-7 dell 权威数字到手**：FINAL 三轴（pack 1×4 / batch
> 4×1 / joint 2×2）全部 **1.60× ± 1% 精确命中行计数理论 1.600×**，旗标增量
> +21%，bodies=8 = 64%（唯一遗留缺口 = WS-9）；**两个真 bug 修复**：库的
> AVX512_OPT 外部缓冲对齐访问（布局依赖段错误，生产 E1 侥幸通过）+ 构建配方
> AVX512_OPT 必须全局（misc.c 对齐策略）。详见 stage420 §四b + stage421 §五b/c。
...
## 6ae. 预对齐联合 packing+batching 定案 + 成本模型三表（2026-09-10 晚，当前最新）

### 定案内容（提交 26663a5 + stage419/420）

1. **正确性完全闭合（stage419）**：800dfad 遗留的 107/256 失败根因 =
   探针 LUT 网格 2× 过细（值边界恰在 modswitch 读取点，两路径 ±1 舍入
   差翻级）——**预对齐机制本身完全正确**。标准 LUT（值跨度 2N/2^prec）
   下全配置 0 失配：r1∈{1,2,4}×r2∈{1,2,4}、h∈{1,3,4,6}、out_N∈
   {2048,4096}、4 seeds、NOALIGN 回归、**FINAL n=2048/h=42 门 0/8192**。
2. **理论机器裁决（stage419 §二）**：方向 A 闭式
   E(t) = −b̄[t] + Σ_{p≤t} ā[(t−p) mod n] − Σ_{p>t} ā[...] + 257 (mod 2N)
   对 oracle 与联合两路径全槽精确成立（±分裂 = 负循环读取绕回符号，
   与 bpa 构造逐项一致；+257 为路径无关常数，在路径差中消去）。
   stage418 §七方向 B 公式符号书写已加勘误头。
3. **成本模型与实测（stage420）**：行计数定律——EP 数全变体相同
   (h+1)ρn，每 EP 行数 k+bodies；预对齐使 sub_a = 明文单项式（零
   EP），行模型精确成立。渐近 r→∞ 加速上限 2×（k+1→1）。实测三表
   （WSL 同构建 6 reps 中位数）：
   - pack：r2=2 **1.185×**、r2=4 **1.249×**（理论 1.333/1.600）；
     生产旗标路径历史数字 1.21×（binary）/1.81×（全旗标）。
   - batch：r1=2 **1.194×**、r1=4 **1.242×**——与 pack 轴逐点对称，
     预对齐批处理归约到打包行模型的直接证据；interleaved Hom-Tr
     对照 0.775×（每站 2 aut-KS 正开销，机制一致）→ 降为备选。
   - 联合：2×2 玩具 **1.227×**、FINAL **1.300×**（vs 标量，理论
     1.600×）；dell stage417 直接 packing 基线 1.197× vs 理论
     1.200×（0.3% 误差）。bodies=8 实现率降至 ~58%——未优化通用
     mat-EP 的行扩展效率，修复路径 = 接入 R4/R6 行瓦片旗标内核。
4. **仪器沉淀**：probe_prealigned2 获 SAB_PA_TRACE（终端指数提取）、
   SAB_PA_RPREC、SAB_PA_LUT_FINE、setup 计时与 speedup 输出；
   WSL 构建 build_wsl/pa2（正确性+相对口径；绝对口径仍 dell）。

### 剩余义务（新优先序）

1. **联合管线接入行瓦片化 EP 内核**（R6_FULLTILE 等）后重测 bodies=8
   ——当前唯一实质性能缺口（stage420 §四.2）。
2. **KS 版 b′ 真实门**：预对齐从明文模拟换为真 keyswitch（理论低阶，
   stage419 §六.2）；同时补 vs 分离 packing 的直接计时臂（新探针下）。
3. **+257 常数形式化推导**（stage419 §六.1；不影响正确性主张）。
