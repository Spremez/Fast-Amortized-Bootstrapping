...
D-1/D-2/D-3 全部 CLOSED。**stage400 进度 15/22，剩余：P-3（§6-7
装配）→ P-4（附录 B/C/D + 工件脱敏打包）→ P-5（记号 lint +
Bergerat 措辞 + 两版 diff 冻结）→ D-4（内审两轮：数学审稿人 10 项
攻击清单重放 + 实验统计口径）。可选加分：I-5（r₁⊗r₂ 玩具门）、
I-6（FINAL 参数 profiler + σ_kg 重标定）。全部素材已就绪。**
