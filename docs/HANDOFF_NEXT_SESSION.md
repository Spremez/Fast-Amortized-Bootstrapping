...
>
> **⚠️ 2026-09-11 更新（WS-8/WS-9 定位）**：**+257 常数定律闭合**（stage419 §七）：
> C = 2^(log₂(2N)−prec−1) + 1 = modswitch(po) + 1，四参数点（out_N 2048/4096/1024 ×
> prec 2/3/5）× 双路径 × 256 槽全部精确；定理重述为 E(t) ≈ −modswitch(φ)+1（结构
> +1 的 S1 机制解释标注为待形式化）。**WS-9 定位完成**（stage421 §五d）：每行成本
> 在 2-5 行持平、9 行跳升 +34%——行瓦片 EP 内核扩展是唯一剩余性能工作
> （bodies=8：64%→85%+ 目标）。此后即 WS-10 论文并入（素材全齐）。
