...
### P0-b 重大修正 + goal 调整建议（2026-08-28 第三波）

**误报修正**: 非零个数扫描显示"泄漏"随 k 平滑增长（4→52.67 … 2048→57.94 bit raw，无悬崖）→ 量级复核: 探针偏差 = 2^-7..-12 torus（低于 2^-4 消息预算），而真实 bind 垃圾 = ¼ 尺度 2^62（消息级）。**"实验 B 命中"是误报**——probe_envneg 复现的是参照系 vs 信封在噪声地板级的语义分歧，非 bind bug。SQ 内核的端到端 bootstrap 通过（数百万次同构乘积）始终是信封正确性的更强证据。

**推论**: 探针代理路线对 P0-b 死路（尺度差 10 bit）。剩余路线: (i) 真 bind 原位逐层 dump（timebox 1 会话）; (ii) 战略降级 D4 为论文 discussion（D0-D3 理论工件仍可引）。

**Goal 调整建议（呈作者裁定）**: P0-b 降为次优先（1 会话 timebox），**stage357（SQ×r-lane 两岛合并）升为主优先**——审计确认的最大未叠加优势、机械性移植、直接合成论文双主结果（1.52-1.83× × 10.2%）。Eurocrypt 时间线支持此序。
