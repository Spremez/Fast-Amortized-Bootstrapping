...
int main(int argc, char ** argv){
  ...
  if(mode & 8){
     ...
     * drives the FFT dynamic-range cliff). */
    {
      const int thr = (argc > 3) ? atoi(argv[3]) : 0;
      if(thr > 0){
        const int32_t lim = (int32_t)(((uint32_t)1) << (thr - 1));
        for (int q = 0; q < N; q++){
          const int32_t sv = (int32_t)(uint32_t) dig->coeffs[q];
          if(sv > -lim && sv < lim) dig->coeffs[q] = 0;
        }
      }
    }
  }else if(mode & 2){
