...
static uint64_t sab_pvw_sq_now_us(void){
  struct timeval tv;
...
#define SAB_PVW_POSTPROC_TIME_ACC(ACC, CODE) \
  do { \
    const uint64_t __sab_pvw_sq_begin = sab_pvw_sq_now_us(); \
    CODE; \
    (ACC) += sab_pvw_sq_now_us() - __sab_pvw_sq_begin; \
  } while (0)
#else
#define SAB_PVW_POSTPROC_TIME_ACC(ACC, CODE) \
  do { \
...
static SAB_PVW_Body_Profile sab_pvw_sq_body_profile;

static void sab_pvw_sq_body_profile_reset(void){
  memset(&sab_pvw_sq_body_profile, 0, sizeof(sab_pvw_sq_body_profile));
}

static void sab_pvw_sq_body_profile_acc(uint64_t * us, uint64_t * calls,
    uint64_t begin_us){
  *us += sab_pvw_sq_now_us() - begin_us;
  (*calls)++;
}
...
static void sab_pvw_sq_body_profile_print(SAB_PVW_SQ_Key sab, uint64_t full_us){
  printf("SAB_PVW_BODY_PROFILE sample lanes=%" PRIu64
         ...
         sab_pvw_sq_body_profile.setup_tv_xb_calls,
         sab_pvw_sq_body_profile.setup_tv_xb_us,
         sab_pvw_sq_body_profile.blind_rotate_calls,
         sab_pvw_sq_body_profile.blind_rotate_us,
         sab_pvw_sq_body_profile.sparse_mul_calls,
         sab_pvw_sq_body_profile.sparse_mul_us,
         sab_pvw_sq_body_profile.rgsw_monomial_calls,
         sab_pvw_sq_body_profile.rgsw_monomial_us,
         sab_pvw_sq_body_profile.cmux_calls,
         sab_pvw_sq_body_profile.cmux_us,
         sab_pvw_sq_body_profile.ncmux_calls,
         sab_pvw_sq_body_profile.ncmux_us,
         sab_pvw_sq_body_profile.cmux_sub_calls,
         sab_pvw_sq_body_profile.cmux_sub_us,
         sab_pvw_sq_body_profile.cmux_from_dft_calls,
         sab_pvw_sq_body_profile.cmux_from_dft_us,
         sab_pvw_sq_body_profile.cmux_add_calls,
         sab_pvw_sq_body_profile.cmux_add_us,
         sab_pvw_sq_body_profile.ncmux_auto_calls,
         sab_pvw_sq_body_profile.ncmux_auto_us,
         sab_pvw_sq_body_profile.mat_ep_calls,
         sab_pvw_sq_body_profile.mat_ep_us,
         sab_pvw_sq_body_profile.sub_a_calls,
         sab_pvw_sq_body_profile.sub_a_us,
         sab_pvw_sq_body_profile.sub_a_output_fusion_calls,
         sab_pvw_sq_body_profile.sub_a_output_fusion_us,
         sab_pvw_sq_body_profile.sub_a_rotate_calls,
         sab_pvw_sq_body_profile.sub_a_rotate_us,
         sab_pvw_sq_body_profile.sub_a_mul_minus_1_calls,
         sab_pvw_sq_body_profile.sub_a_mul_minus_1_us,
         sab_pvw_sq_body_profile.sub_a_copy_calls,
         sab_pvw_sq_body_profile.sub_a_copy_us,
         sab_pvw_sq_body_profile.sub_a_mat_ep_calls,
         sab_pvw_sq_body_profile.sub_a_mat_ep_us,
         sab_pvw_sq_body_profile.sub_a_from_dft_calls,
         sab_pvw_sq_body_profile.sub_a_from_dft_us,
         sab_pvw_sq_body_profile.sub_a_add_calls,
         sab_pvw_sq_body_profile.sub_a_add_us,
         sab_pvw_sq_body_profile.dual_sub_pair_calls,
         sab_pvw_sq_body_profile.dual_sub_pair_us,
         sab_pvw_sq_body_profile.schedule_fused_cmux_calls,
         sab_pvw_sq_body_profile.schedule_fused_ncmux_calls,
         sab_pvw_sq_body_profile.copyback_calls,
         sab_pvw_sq_body_profile.copyback_us,
  ...
  for (size_t bit = 0; bit < (size_t) bits; bit++){
    const uint64_t total_update_calls =
        sab_pvw_sq_body_profile.bit_ncmux_calls[bit] +
        sab_pvw_sq_body_profile.bit_direct_cmux_calls[bit];
    printf(" bit%zu_ncmux_calls=%" PRIu64
           ...
           bit, sab_pvw_sq_body_profile.bit_ncmux_calls[bit],
           bit, sab_pvw_sq_body_profile.bit_ncmux_us[bit],
           bit, sab_pvw_sq_body_profile.bit_direct_cmux_calls[bit],
           bit, sab_pvw_sq_body_profile.bit_direct_cmux_us[bit],
...
static void sab_pvw_sq_die(const char * msg){
...
static inline PVW_TMLWE * sab_pvw_sq_accumulator_active(
    const SAB_PVW_Accumulator_State * state){
  return state->buffers[state->active];
}

static inline PVW_TMLWE * sab_pvw_sq_accumulator_inactive(
    const SAB_PVW_Accumulator_State * state){
  return state->buffers[state->active ^ 1];
}

static inline void sab_pvw_sq_accumulator_flip(
    SAB_PVW_Accumulator_State * state){
  state->active ^= 1;
}

static void sab_pvw_sq_encrypt_bits(MAT_TRGSW_DFT * out, MAT_TRGSW tmp,
...
static inline int sab_pvw_sq_coeff_is_minus_one(uint64_t coeff){
  return coeff == (uint64_t) -1;
}

static MAT_TRGSW_DFT * sab_pvw_sq_alloc_selector_bits(uint64_t r_prec,
...
static TRLWE_Key sab_pvw_sq_trlwe_key_from_lane(PVW_TMLWE_Key in, uint64_t lane){
...
static void sab_pvw_sq_materialize_pvwtlwe_lane(TLWE out, PVW_TLWE in,
    uint64_t lane){
...
static void sab_pvw_sq_extract_tlwe_lane(TLWE out, PVW_TMLWE in,
...
static void sab_pvw_sq_init_full_postproc(SAB_PVW_SQ_Key sab, TRLWE_Key input_key,
  ...
  for (size_t lane = 0; lane < sab->lanes; lane++){
    TRLWE_Key lane_key = sab_pvw_sq_trlwe_key_from_lane(sab->output_key, lane);
...
SAB_PVW_SQ_Key sab_pvw_sq_new_binary_key(TRLWE_Key input_key, PVW_TMLWE_Key output_key,
  ...
  for (size_t key_idx = 0; key_idx < in_k; key_idx++){
    ...
    for (size_t scan = 0; scan < in_N; scan++){
      ...
      if(coeff == 0) continue;
      if(coeff != 1) sab_pvw_sq_die("only binary sparse input keys are supported");
      if(cnt_h >= h) sab_pvw_sq_die("input key has more non-zero coefficients than h");
      const uint64_t r_diff = previous - current;
      if(r_diff >= r_max) sab_pvw_sq_die("input key monomial gap exceeds r_prec");
      res->s[key_idx][cnt_h] = sab_pvw_sq_alloc_selector_bits(r_prec, l,
          bg_bit, out_k, lanes, out_N);
      sab_pvw_sq_encrypt_bits(res->s[key_idx][cnt_h], tmp, res->mat_key,
          r_diff, r_prec);
      previous = current;
      cnt_h++;
    }
    if(cnt_h != h) sab_pvw_sq_die("input key has fewer non-zero coefficients than h");
    if(previous >= r_max) sab_pvw_sq_die("final monomial gap exceeds r_prec");
    res->s[key_idx][cnt_h] = sab_pvw_sq_alloc_selector_bits(r_prec, l,
        bg_bit, out_k, lanes, out_N);
    sab_pvw_sq_encrypt_bits(res->s[key_idx][cnt_h], tmp, res->mat_key,
        previous, r_prec);
  ...
  res->tmp = (sab_pvw_sq_tmp_pool) safe_malloc(sizeof(*res->tmp));
...
SAB_PVW_SQ_Key sab_pvw_sq_new_nonbinary_key(TRLWE_Key input_key,
  ...
  if(include_zeros && ternary){
    sab_pvw_sq_die("include-zero and ternary PVW modes must be tested separately");
  }
  if(!include_zeros && !ternary){
    return sab_pvw_sq_new_binary_key(input_key, output_key, b_prec, h,
        r_prec, l, bg_bit);
  }
  ...
  for (size_t key_idx = 0; key_idx < in_k; key_idx++){
    ...
    for (size_t scan = 0; scan < in_N; scan++){
      ...
      if(include_zeros && coeff != 1){
        sab_pvw_sq_die("include-zero PVW sparse input expects coefficient one");
      }
      if(ternary && coeff != 1 && !sab_pvw_sq_coeff_is_minus_one(coeff)){
        sab_pvw_sq_die("ternary PVW sparse input expects +/-1 coefficients");
      }
      if(cnt_h >= h) sab_pvw_sq_die("input key has more non-zero coefficients than h");
      const uint64_t r_diff = previous - current;
      if(r_diff >= r_max){
        sab_pvw_sq_die("input key monomial gap exceeds r_prec");
      }
      res->s[key_idx][cnt_h] = sab_pvw_sq_alloc_selector_bits(r_prec, l,
          bg_bit, out_k, lanes, out_N);
      sab_pvw_sq_encrypt_bits(res->s[key_idx][cnt_h], tmp, res->mat_key,
      ...
      if(ternary){
        ...
        mat_trgsw_monomial_DFT_sample(res->s_sign[key_idx][cnt_h],
            sab_pvw_sq_coeff_is_minus_one(coeff) ? 1 : 0, 0, res->mat_key);
      }
      previous = current;
      cnt_h++;
    }
...
SAB_PVW_SQ_Key sab_pvw_sq_new_binary_full_key(TRLWE_Key input_key,
  ...
  /* stage357 SQ: the scale-quantized kernel requires l = 1 with the gadget
   * base replaced by the accumulator quantization q (message at 2^(64-q)).
   * Caller values are overridden; q from SAB_PVW_SQ_Q (default 16). */
  l = 1;
  {
    const char * e = getenv("SAB_PVW_SQ_Q");
...
static void sab_pvw_sq_CMUX_materialize_internal(PVW_TMLWE out,
  ...
  if(prefer_fused_from_dft_add && out != addend){
    sab_pvw_sq_rescale_add(out, sab->tmp->tmlwe_dft, addend, sab);
#if 0
#ifdef SAB_PVW_BODY_PROFILE
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.cmux_from_dft_us,
        &sab_pvw_sq_body_profile.cmux_from_dft_calls, cmux_from_dft_begin);
    sab_pvw_sq_body_profile.cmux_add_calls++;
#endif
#endif
  }else{
    ...
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.cmux_from_dft_us,
        &sab_pvw_sq_body_profile.cmux_from_dft_calls, cmux_from_dft_begin);
    const uint64_t cmux_add_begin = sab_pvw_sq_now_us();
#endif
    pvmtmlwe_add(out, sab->tmp->tmlwe, addend);
#ifdef SAB_PVW_BODY_PROFILE
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.cmux_add_us,
        &sab_pvw_sq_body_profile.cmux_add_calls, cmux_add_begin);
#endif
#endif
  }
}
...
static void sab_pvw_sq_rescale_add(PVW_TMLWE out, PVW_TMLWE_DFT prod,
  ...
  for (int c = 0; c < N; c++)
    out->a[0]->coeffs[c] = addend->a[0]->coeffs[c]
        + (Torus)((((int64_t) tmpa->coeffs[c]) + half) >> shift);
  for (int j = 0; j < addend->r; j++)
    for (int c = 0; c < N; c++)
      out->b[j]->coeffs[c] = addend->b[j]->coeffs[c]
          + (Torus)((((int64_t) tmpb[j]->coeffs[c]) + half) >> shift);
}
...
static void sab_pvw_sq_CMUX_from_sub_internal(PVW_TMLWE out, PVW_TMLWE addend,
  ...
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.mat_ep_us,
      &sab_pvw_sq_body_profile.mat_ep_calls, mat_ep_begin);
#endif
  sab_pvw_sq_CMUX_materialize_internal(out, addend, sab,
...
#endif
#ifdef SAB_PVW_SUB_DECOMP_FUSION
  sab_pvw_sq_CMUX_from_diff_internal(out, in1, in1, in2, selector, sab,
      prefer_fused_from_dft_add);
#else
#ifdef SAB_PVW_BODY_PROFILE
  const uint64_t cmux_sub_begin = sab_pvw_sq_now_us();
#endif
  pvmtmlwe_sub(sab->tmp->tmlwe, in2, in1);
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.cmux_sub_us,
      &sab_pvw_sq_body_profile.cmux_sub_calls, cmux_sub_begin);
#endif
  sab_pvw_sq_CMUX_from_sub_internal(out, in1, sab->tmp->tmlwe,
      selector, sab, prefer_fused_from_dft_add);
#endif
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.cmux_us,
      &sab_pvw_sq_body_profile.cmux_calls, cmux_begin);
#endif
}
...
#else
  sab_pvw_sq_CMUX_internal(out, in1, in2, selector, sab, 0);
#endif
}
...
#endif
  /* SQ: exact up-shift to torus scale around the stock automorphism KS,
   * then round back to Q-scale (KS semantics/noise unchanged). */
  {
    const unsigned up = 64 - sab->q;
    PVW_TMLWE t = sab->tmp->rotated;
    const int rr = in2->r, N = in2->b[0]->N;
    for (int c = 0; c < N; c++){
      t->a[0]->coeffs[c] = (Torus)(((int64_t) in2->a[0]->coeffs[c]) << up);
      for (int j = 0; j < rr; j++)
        t->b[j]->coeffs[c] = (Torus)(((int64_t) in2->b[j]->coeffs[c]) << up);
    }
    pvmtmlwe_eval_automorphism(sab->tmp->tmlwe, t, 2 * N - 1, sab->aut_minus1);
    const unsigned sh2 = 64 - sab->q;
    const int64_t half = 1LL << (sh2 - 1);
    PVW_TMLWE u = sab->tmp->tmlwe;
    for (int c = 0; c < N; c++){
      t->a[0]->coeffs[c] = (Torus)((((int64_t) u->a[0]->coeffs[c]) + half) >> sh2);
      for (int j = 0; j < rr; j++)
        t->b[j]->coeffs[c] = (Torus)((((int64_t) u->b[j]->coeffs[c]) + half) >> sh2);
    }
  }
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.ncmux_auto_us,
      &sab_pvw_sq_body_profile.ncmux_auto_calls, ncmux_auto_begin);
#endif
  sab_pvw_sq_CMUX(out, in1, sab->tmp->rotated, selector, sab);
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.ncmux_us,
      &sab_pvw_sq_body_profile.ncmux_calls, ncmux_begin);
#endif
}
...
static void sab_pvw_sq_schedule_NCMUX(PVW_TMLWE out, PVW_TMLWE in1,
  ...
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.ncmux_auto_us,
      &sab_pvw_sq_body_profile.ncmux_auto_calls, ncmux_auto_begin);
#endif
  sab_pvw_sq_CMUX_internal(out, in1, sab->tmp->rotated, selector, sab, 1);
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.ncmux_us,
      &sab_pvw_sq_body_profile.ncmux_calls, ncmux_begin);
#endif
...
static void sab_pvw_sq_dual_sub_shared(PVW_TMLWE out_rot_minus_shared,
...
static void sab_pvw_sq_schedule_dual_sub_pair(PVW_TMLWE out_ncmux,
  ...
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.ncmux_auto_us,
      &sab_pvw_sq_body_profile.ncmux_auto_calls, ncmux_auto_begin);
  const uint64_t dual_sub_begin = sab_pvw_sq_now_us();
#endif
  sab_pvw_sq_dual_sub_shared(sab->tmp->tmlwe, sab->tmp->rotated,
      sab->tmp->rotated, shared, direct_rhs);
#ifdef SAB_PVW_BODY_PROFILE
  const uint64_t dual_sub_elapsed = sab_pvw_sq_now_us() - dual_sub_begin;
  sab_pvw_sq_body_profile.cmux_sub_us += dual_sub_elapsed;
  sab_pvw_sq_body_profile.cmux_sub_calls += 2;
  sab_pvw_sq_body_profile.dual_sub_pair_us += dual_sub_elapsed;
  sab_pvw_sq_body_profile.dual_sub_pair_calls++;
  const uint64_t ncmux_cmux_begin = sab_pvw_sq_now_us();
#endif
  sab_pvw_sq_CMUX_from_sub_internal(out_ncmux, shared, sab->tmp->tmlwe,
      selector, sab, 1);
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.cmux_us,
      &sab_pvw_sq_body_profile.cmux_calls, ncmux_cmux_begin);
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.ncmux_us,
      &sab_pvw_sq_body_profile.ncmux_calls, ncmux_begin);
  const uint64_t direct_cmux_begin = sab_pvw_sq_now_us();
#endif
  sab_pvw_sq_CMUX_from_sub_internal(out_direct, direct_rhs, sab->tmp->rotated,
      selector, sab, 1);
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.cmux_us,
      &sab_pvw_sq_body_profile.cmux_calls, direct_cmux_begin);
#endif
}
#endif
  ...
  for (size_t bit = 0; bit < r_prec; bit++){
...
#ifdef SAB_PVW_BODY_PROFILE
    const uint64_t ncmux_loop_begin = sab_pvw_sq_now_us();
#endif
#ifdef SAB_PVW_DUAL_SUB_CMUX
    if(2 * power <= in_N){
      for (size_t j = 0; j < power; j++){
        sab_pvw_sq_schedule_dual_sub_pair(p[out][j], p[out][j + power],
...
#endif
    {
      for (size_t j = 0; j < power; j++){
        ...
        sab_pvw_sq_schedule_NCMUX(p[out][j], p[in][j], p[in][in_N - power + j],
            e[bit], sab);
#else
        sab_pvw_sq_NCMUX(p[out][j], p[in][j], p[in][in_N - power + j],
            e[bit], sab);
#endif
      }
    }
    ...
    if(bit < SAB_PVW_BODY_PROFILE_MAX_BITS){
      sab_pvw_sq_body_profile.bit_ncmux_us[bit] +=
          sab_pvw_sq_now_us() - ncmux_loop_begin;
      sab_pvw_sq_body_profile.bit_ncmux_calls[bit] += power;
    }
    const uint64_t direct_cmux_loop_begin = sab_pvw_sq_now_us();
#endif
    for (size_t j = direct_start; j < in_N - power; j++){
#ifdef SAB_PVW_SCHEDULE_FUSED_CMUX
      sab_pvw_sq_schedule_CMUX(p[out][j + power], p[in][j + power], p[in][j],
          e[bit], sab);
#else
      sab_pvw_sq_CMUX(p[out][j + power], p[in][j + power], p[in][j],
          e[bit], sab);
#endif
    }
#ifdef SAB_PVW_BODY_PROFILE
    if(bit < SAB_PVW_BODY_PROFILE_MAX_BITS){
      sab_pvw_sq_body_profile.bit_direct_cmux_us[bit] +=
          sab_pvw_sq_now_us() - direct_cmux_loop_begin;
      sab_pvw_sq_body_profile.bit_direct_cmux_calls[bit] += in_N - power;
    }
#endif
  }
  active ^= r_prec & 1;
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.rgsw_monomial_us,
      &sab_pvw_sq_body_profile.rgsw_monomial_calls, rgsw_begin);
#endif
  return active;
}
...
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.copyback_us,
      &sab_pvw_sq_body_profile.copyback_calls, copyback_begin);
#endif
}
...
static inline void sab_pvw_sq_accumulator_normalize(
  ...
  if(state->active != 0){
    sab_pvw_sq_copy_accumulator_array(state->buffers[0],
        sab_pvw_sq_accumulator_active(state), sab);
    state->active = 0;
  }
}
...
void sab_pvw_sq_sub_a_binary(PVW_TMLWE * p, const uint64_t * a, SAB_PVW_SQ_Key sab){
  ...
  for (size_t idx = 0; idx < sab->in_N; idx++){
#ifdef SAB_PVW_BODY_PROFILE
    const uint64_t rotate_begin = sab_pvw_sq_now_us();
#endif
    pvmtmlwe_mul_by_xai(sab->tmp->tmlwe, p[idx], a[idx]);
#ifdef SAB_PVW_BODY_PROFILE
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.sub_a_rotate_us,
        &sab_pvw_sq_body_profile.sub_a_rotate_calls, rotate_begin);
    const uint64_t copy_begin = sab_pvw_sq_now_us();
#endif
    pvmtmlwe_copy(p[idx], sab->tmp->tmlwe);
#ifdef SAB_PVW_BODY_PROFILE
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.sub_a_copy_us,
        &sab_pvw_sq_body_profile.sub_a_copy_calls, copy_begin);
#endif
  }
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.sub_a_us,
      &sab_pvw_sq_body_profile.sub_a_calls, sub_a_begin);
#endif
}
...
void sab_pvw_sq_sub_a_include_zero(PVW_TMLWE * p, const uint64_t * a,
  ...
  for (size_t idx = 0; idx < sab->in_N; idx++){
#ifdef SAB_PVW_BODY_PROFILE
    const uint64_t mul_minus_1_begin = sab_pvw_sq_now_us();
#endif
    pvmtmlwe_mul_by_xai_minus_1(sab->tmp->tmlwe, p[idx], a[idx]);
#ifdef SAB_PVW_BODY_PROFILE
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.sub_a_mul_minus_1_us,
        &sab_pvw_sq_body_profile.sub_a_mul_minus_1_calls, mul_minus_1_begin);
    const uint64_t mat_ep_begin = sab_pvw_sq_now_us();
#endif
    mat_trgsw_mul_pvmtmlwe_DFT(sab->tmp->tmlwe_dft, sab->tmp->tmlwe,
        selector, sab->tmp->scratch);
#ifdef SAB_PVW_BODY_PROFILE
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.sub_a_mat_ep_us,
        &sab_pvw_sq_body_profile.sub_a_mat_ep_calls, mat_ep_begin);
    const uint64_t from_dft_begin = sab_pvw_sq_now_us();
#endif
#ifdef SAB_PVW_SUBA_FUSED_FROM_DFT_ADD
    pvmtmlwe_from_DFT_add(p[idx], sab->tmp->tmlwe_dft, p[idx]);
#ifdef SAB_PVW_BODY_PROFILE
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.sub_a_from_dft_us,
        &sab_pvw_sq_body_profile.sub_a_from_dft_calls, from_dft_begin);
#endif
#else
    pvmtmlwe_from_DFT(sab->tmp->tmlwe, sab->tmp->tmlwe_dft);
#ifdef SAB_PVW_BODY_PROFILE
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.sub_a_from_dft_us,
        &sab_pvw_sq_body_profile.sub_a_from_dft_calls, from_dft_begin);
    const uint64_t add_begin = sab_pvw_sq_now_us();
#endif
    pvmtmlwe_addto(p[idx], sab->tmp->tmlwe);
#ifdef SAB_PVW_BODY_PROFILE
    sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.sub_a_add_us,
        &sab_pvw_sq_body_profile.sub_a_add_calls, add_begin);
#endif
#endif
  }
#endif
...
#ifdef SAB_PVW_BODY_PROFILE
  const uint64_t elapsed = sab_pvw_sq_now_us() - sub_a_begin;
  sab_pvw_sq_body_profile.sub_a_us += elapsed;
  sab_pvw_sq_body_profile.sub_a_calls++;
  sab_pvw_sq_body_profile.sub_a_output_fusion_us += elapsed;
  sab_pvw_sq_body_profile.sub_a_output_fusion_calls++;
#endif
}
...
#endif
  if(a_idx >= sab->in_k) sab_pvw_sq_die("sparse_mul a_idx out of range");
#ifdef SAB_PVW_ACTIVE_BUFFER_FUSION
  SAB_PVW_Accumulator_State state = sab_pvw_sq_accumulator_state(p, sab);
  for (size_t step = 0; step < sab->h; step++){
    state.active = sab_pvw_sq_RGSW_monomial_mul_state(state.buffers,
        state.active,
        sab->s[a_idx][step], sab);
#ifdef SAB_PVW_SUBA_OUTPUT_FUSION
    sab_pvw_sq_sub_a_binary_to(sab_pvw_sq_accumulator_inactive(&state),
        sab_pvw_sq_accumulator_active(&state), a, sab);
    sab_pvw_sq_accumulator_flip(&state);
#else
    sab_pvw_sq_sub_a_binary(sab_pvw_sq_accumulator_active(&state), a, sab);
#endif
  }
  state.active = sab_pvw_sq_RGSW_monomial_mul_state(state.buffers,
      state.active,
...
#else
  for (size_t step = 0; step < sab->h; step++){
    sab_pvw_sq_RGSW_monomial_mul(p, sab->s[a_idx][step], sab);
    sab_pvw_sq_sub_a_binary(p, a, sab);
  }
  sab_pvw_sq_RGSW_monomial_mul(p, sab->s[a_idx][sab->h], sab);
#endif
#ifdef SAB_PVW_BODY_PROFILE
  sab_pvw_sq_body_profile_acc(&sab_pvw_sq_body_profile.sparse_mul_us,
      &sab_pvw_sq_body_profile.sparse_mul_calls, sparse_mul_begin);
#endif
}
...
void sab_pvw_sq_sparse_mul_nonbinary(PVW_TMLWE * p, const uint64_t * a,
  ...
  if(sab->include_zeros && sab->s_coff == NULL){
    sab_pvw_sq_die("include-zero sparse_mul selector family is missing");
  }
  if(sab->ternary_secret && sab->s_sign == NULL){
    sab_pvw_sq_die("ternary sparse_mul selector family is missing");
  }
  if(!sab->include_zeros && !sab->ternary_secret){
    sab_pvw_sq_die("nonbinary sparse_mul requires include-zero or ternary mode");
  }
  ...
  for (size_t step = 0; step < sab->h; step++){
    ...
    if(sab->include_zeros){
      sab_pvw_sq_sub_a_include_zero(sab_pvw_sq_accumulator_active(&state),
          a, sab->s_coff[a_idx][step], sab);
    }else{
      sab_pvw_sq_sub_a_ternary(sab_pvw_sq_accumulator_active(&state),
...
void sab_pvw_sq_setup_tv_xb(PVW_TMLWE * acc, const uint64_t * b,
  ...
  const unsigned sh = 64 - sab->q;
  const int64_t half = 1LL << (sh - 1);
  for (size_t idx = 0; idx < sab->in_N; idx++){
    pvmtmlwe_mul_by_xai(acc[idx], tv, torus2int(b[idx] + prec_offset, log_N2));
    PVW_TMLWE p = acc[idx];
    const int N = p->b[0]->N, rr = p->r;
    for (int c = 0; c < N; c++){
      p->a[0]->coeffs[c] = (Torus)((((int64_t) p->a[0]->coeffs[c]) + half) >> sh);
      for (int j = 0; j < rr; j++)
        p->b[j]->coeffs[c] = (Torus)((((int64_t) p->b[j]->coeffs[c]) + half) >> sh);
    }
  }
}
...
void sab_pvw_sq_blind_rotate_binary(PVW_TMLWE * out, TRLWE in, SAB_PVW_SQ_Key sab){
  ...
  for (size_t key_idx = 0; key_idx < sab->in_k; key_idx++){
    ...
    sab_pvw_sq_sparse_mul_binary(out, sab->tmp->a_mod, key_idx, sab);
  }
}
...
void sab_pvw_sq_blind_rotate_nonbinary(PVW_TMLWE * out, TRLWE in,
  ...
  for (size_t key_idx = 0; key_idx < sab->in_k; key_idx++){
    ...
    sab_pvw_sq_sparse_mul_nonbinary(out, sab->tmp->a_mod, key_idx, sab);
  }
}
...
void sab_pvw_sq_bootstrap_wo_extract_binary(PVW_TMLWE * out, TRLWE in,
  ...
  const uint64_t setup_begin = sab_pvw_sq_now_us();
#endif
  sab_pvw_sq_setup_tv_xb(out, in->b->coeffs, tv, sab);
#ifdef SAB_PVW_BODY_PROFILE
  ...
  {
    const unsigned up = 64 - sab->q;
    const int N = out[0]->b[0]->N;
    for (size_t idx = 0; idx < sab->in_N; idx++){
      PVW_TMLWE p = out[idx];
      for (int c = 0; c < N; c++){
        p->a[0]->coeffs[c] = (Torus)(((int64_t) p->a[0]->coeffs[c]) << up);
        for (int j = 0; j < p->r; j++)
          p->b[j]->coeffs[c] = (Torus)(((int64_t) p->b[j]->coeffs[c]) << up);
      }
    }
  }
#ifdef SAB_PVW_BODY_PROFILE
  ...
  sab_pvw_sq_body_profile_print(sab, sab_pvw_sq_now_us() - full_begin);
#endif
}
...
void sab_pvw_sq_bootstrap_binary(TRLWE * out, TRLWE in, PVW_TMLWE tv,
  ...
  SAB_PVW_POSTPROC_TIME_ACC(bootstrap_wo_extract_us,
      sab_pvw_sq_bootstrap_wo_extract_binary(sab->tmp->acc, in, tv, sab));
  for (size_t lane = 0; lane < sab->lanes; lane++){
    SAB_PVW_POSTPROC_TIME_ACC(direct_extract_us,
        sab_pvw_sq_extract_tlwe_lane_array(sab->tmp->lane_extracted[lane],
...
#ifdef SAB_PVW_POSTPROC_PROFILE
  const uint64_t full_us = sab_pvw_sq_now_us() - full_begin;
...
void sab_pvw_sq_bootstrap_nonbinary(TRLWE * out, TRLWE in, PVW_TMLWE tv,
  ...
  if(!sab->include_zeros && !sab->ternary_secret){
    sab_pvw_sq_die("nonbinary bootstrap requires include-zero or ternary mode");
  }
#ifdef SAB_PVW_POSTPROC_PROFILE
  uint64_t bootstrap_wo_extract_us = 0;
  uint64_t direct_extract_us = 0;
...
#endif
  SAB_PVW_POSTPROC_TIME_ACC(bootstrap_wo_extract_us,
      sab_pvw_sq_bootstrap_wo_extract_nonbinary(sab->tmp->acc, in, tv, sab));
  for (size_t lane = 0; lane < sab->lanes; lane++){
    SAB_PVW_POSTPROC_TIME_ACC(direct_extract_us,
        sab_pvw_sq_extract_tlwe_lane_array(sab->tmp->lane_extracted[lane],
            sab->tmp->acc, lane, sab));
