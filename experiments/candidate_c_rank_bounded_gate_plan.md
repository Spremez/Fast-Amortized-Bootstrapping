# Candidate C Rank-Bounded Gate Plan

1. Recompute the C1 operator tensor for `r=2,4,6`.
2. Bind source, equations, symbolic relations, phase, schedule, rank, and
   compression/structural cost in one canonical summary.
3. Preserve `NO_VERIFIED_TASK3B_RESULT` for C2.
4. Preserve `SKIPPED_NO_REGISTERED_OPERATOR` for Task 4 without numeric substitution.
5. Apply `REJECT_CANDIDATE_C_RANK_BOUNDED_STATE_CAMPAIGN_EXHAUSTED` atomically to the research state and ledgers.

This plan closes the finite Candidate C mechanism budget. It does not authorize
production work and does not create Candidate D.

Implementation-input commit: `6a3004cdf0fb5c35aeffe3984512657f8d2856ce`. Reproduce with
`python scripts/run_candidate_c_rank_bounded_gate.py --input-commit 6a3004cdf0fb5c35aeffe3984512657f8d2856ce`.
