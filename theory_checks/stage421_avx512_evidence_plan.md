...
| WS-8 | +257 常数形式化推导（并入正确性定理证明） | ✓ 定律闭合（stage419 §七：C=modswitch(po)+1 四参数点全槽精确；S1 机制解释标注为待形式化） |
| WS-9 | 行瓦片 EP 内核对 bodies=8 的扩展（若 dell 后仍有缺口） | 已定位+快假设否定（§五d：加旗标不解决，需新内核；不阻塞 WS-10） |
| WS-10 | 论文侧并入（§3 算法/§4 噪声/§5 复杂度/§7 实验四章联动改写） | WS-9 后（素材已齐） |
...
bodies=8 = 64%（WS-9 目标，定位见 §五d）。WSL 负增量确认为本机
CPU 特征而非实现问题。

## 五d、WS-9 定位画像（2026-09-11）

边际成本法（WSL plain，joint 蝶形时间，SAB_PA_TRACE 关）：

| bodies | 行/EP | bfly (us) | us/行 | 相对 |
|---|---|---|---|---|
| 1 | 2 | 546,395 | 273k | 1.00 |
| 2 | 3 | 719,538 | 240k | 0.88 |
| 4 | 5 | ~1,386,000 | 277k | 1.01 |
| 8 | 9 | 3,293,579 | 366k | **1.34** |

每行成本在 2-5 行持平，**9 行跳升 +34%**——每行效率悬崖（缓存
工作集/行瓦片失配），非行模型失效。FINAL（dell）64% vs 玩具 56-58%
的差异与算术强度一致（FINAL in_N=2048 → 8× CMUX 数摊薄访存）。
**WS-9 实现方向**：EP 行循环的瓦片化扩展（R4/R6 内核族推广到 9 行
或 9 行拆分执行），预期把 bodies=8 实现率 64% → 85%+。

### WS-9 快假设否定（2026-09-11，dell 同会话 A/B）

"加旗标即可"假设（PATHA + R4_UNROLLED + RGT4_FUSED，使 r=8 走
rgt4_tiled 内核）**被否定**：
- 正确性不受影响（等价/功能门全 PASS，含 FINAL 0/16384）。
- 同会话交替 A/B（玩具）：flag2 ≤ flag 持续成立（2×2：1.19-1.52
  vs 1.25-1.57；4×2：0.96-1.12 vs 1.01-1.13）。
- FINAL：flag2 4×2 = 0.992×、2×2 = 1.394×（跨会话，但方向一致）。
- 原因分析：rgt4_tiled 内核为 r-lane packing 场景（r=6/8 lane +
  BODYMAJOR 族布局）调优，与本管线的 DUAL_SUB_CMUX 操作数布局
  不匹配；R4_UNROLLED 对本形状也无增益。

**结论**：flag（pathA 七旗标）仍是最优现役层级；bodies=8 的 64%
缺口只能靠**新内核开发**（面向 (1+8)=9 行 PVW 布局的 body-major
瓦片或通用行循环的缓存分块）。这是独立的工程会话（对标 stage84-111
的内核族开发流程），不阻塞 WS-10 论文并入（论文以 bodies=4 的
1.60×±1% 三轴数据为主张锚点，bodies=8 如实标注为工程进行中）。
实现问题。
